<?php
    ...
    if (isset($_GET['delete_confirm']))  {
        ...
        $sql = "delete from pet_adoption where pet_id = ".$pet_id;
        

        if (mysqli_query($connect, $sql)) {

            $sql = "delete from animal where id = ".$pet_id;

            if (mysqli_query($connect, $sql)) {

                $layout .= "<div class='alert alert-success' role='alert'>
                Adoption Case has been deleted!
                </div>";
...
<body>
    ...
    <div>
        <h2 class="text-primary">Detail</h2>
            ...
            <a href="delete.php?pet_id=<?=$pet_id?>&delete_confirm=yes" class="btn btn-danger">Delete Adoption Case!</a>
